...
async function computeFacts() {
  ...
  const [rentDue, headcount, beds, complaints, clTotal, clYesterday, pendingApprovals, pendingClaims] = await Promise.all([
    ...
    pool.query(`SELECT status, category, COUNT(*)::int AS n FROM complaints WHERE status!='resolved' GROUP BY status, category`),
...
const ASK_TEMPLATES = [
  ...
  { id: 'complaints', test: /complain|issue|problem|maintenance/i,
    run: async () => { const r = await pool.query(`SELECT category, COUNT(*)::int AS n FROM complaints WHERE status!='resolved' GROUP BY category ORDER BY n DESC`); const t = r.rows.reduce((a, b) => a + b.n, 0); return t ? `${t} open issue${t === 1 ? '' : 's'}: ` + r.rows.map(x => `${x.category} ${x.n}`).join(', ') + '.' : 'No open issues ✅'; } },
